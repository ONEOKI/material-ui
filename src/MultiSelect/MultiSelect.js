...
class MultiSelect extends Component {
  ...
  handleRequestDelete = (val) => {
    return () => {
      const value = this.state.value.filter((v) => v !== val);
  ...
  render() {
    ...
    if (value && value.length > 0) {
      const valueChips = value.map((val) => (
        <Chip
          style={styles.chip}
...
MultiSelect.levenshteinDistance = (searchText, key) => {
  const current = [];
  let prev;
  let value;
...
MultiSelect.noFilter = () => true;

MultiSelect.defaultFilter = MultiSelect.caseSensitiveFilter = (searchText, key) => {
  return searchText !== '' && key.indexOf(searchText) !== -1;
};

MultiSelect.caseInsensitiveFilter = (searchText, key) => {
  return key.toLowerCase().indexOf(searchText.toLowerCase()) !== -1;
};

MultiSelect.levenshteinDistanceFilter = (distanceLessThan) => {
  if (distanceLessThan === undefined) {
    return MultiSelect.levenshteinDistance;
  } else if (typeof distanceLessThan !== 'number') {
    throw 'Error: MultiSelect.levenshteinDistanceFilter is a filter generator, not a filter!';
  }

  return (s, k) => MultiSelect.levenshteinDistance(s, k) < distanceLessThan;
};

MultiSelect.fuzzyFilter = (searchText, key) => {
...
MultiSelect.Item = MenuItem;
MultiSelect.Divider = Divider;
